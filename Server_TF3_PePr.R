...
PEP_PEAK_WIDTH_TF <- 100 # peak_width for find_overlap in TF case
...
simulation_name_tag <- if (IS_TF_SIMULATION) "TF3_PePr_Sim" else "HM1_PePr_Sim"
...
if (IS_TF_SIMULATION) {
  ...
  sim_params <- list(
    fraglen = 100,      # Fragment length for simulation (distinct from PePr's fraglen for shift)
    base.mu = 120,
    up.mu = 180,
    down.mu.1 = 60,
    down.mu.2 = 60,
