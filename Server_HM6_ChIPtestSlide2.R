...
simulation_name_tag <- if (IS_TF_SIMULATION) "TF_ChIPtestSlide2" else "HM6_ChIPtestSlide2"
...
if (IS_TF_SIMULATION) {
  ...
  sim_params <- list(
    ...
    base.mu = 30,          # Example
    up.mu = 90,            # Example
    down.mu.1 = 10,        # Example for group A's "down" regions compared to baseline for other group
    down.mu.2 = 10,        # Example for group B's "down" regions compared to baseline for other group
  ...
  analysis_peak_width <- 1000 # peak_width for run_chiptest_slide2_analysis when is.tf=FALSE
...
for (iter in 1:num_iterations) {
  ...
  tryCatch({
    ...
    analysis_args <- list(
      ...
      # chiptest_params = specific_chiptest_params_for_run # Uncomment if you have custom ones
    )
