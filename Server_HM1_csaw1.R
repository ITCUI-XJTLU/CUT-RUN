...
window_widths_to_test <- c(150)

# --- Simulation Parameters (customize based on IS_TF_SIMULATION) ---
simulation_name_prefix <- if (IS_TF_SIMULATION) "TF" else "HM1"
...
if (IS_TF_SIMULATION) {
  ...
  sim_params_all_iterations <- list(
    ...
    down.mu.1 = 10,
    down.mu.2 = 10,
    bins_vis = 25,        # For visualization part within simulate_tf
    count_length_vis = 120 # For visualization part
  )
